  ...
  public synchronized Connection getConnection()
  {
    ...
    while (!closed && active == null && nRetries < maxRetries)
    {
      if (retryDelay > 0 && !firstConnectAttempt)
      {
        try
        {
          if (debug) System.out.println(dbg() + ", nRetries=" + nRetries + ", waiting " + retryDelay + " ms ...");
          wait(retryDelay);
      ...
      if (debug) System.out.println(dbg() + ", nRetries=" + nRetries + ", attempt to create connection to: " + entry);
