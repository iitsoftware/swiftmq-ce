...
public class SwiftletDeployer implements DeployListener
{
    ...
    } else
    {
      for (int i = 0; i < cmdList.size(); i++)
      {
        ...
        if (cmd != null && cmd.trim().length() > 0)
        {
          ...
          if (traceSpace.enabled) traceSpace.trace("SwiftletManager", toString() + "/executeCLICommands: " + cmd);
          try
          {
            cliexec.execute(cmd);
          } catch (Exception e)
  ...
  public void bundleAdded(Bundle bundle) throws Exception
  {
    ...
    if (ncmd > 0)
    {
      if (traceSpace.enabled) traceSpace.trace("SwiftletManager", toString() + "/bundleAdded, saving configuration");
