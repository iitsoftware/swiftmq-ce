...
public class IntraVMScheduler
{
  ...
  public synchronized void connectListener(String swiftletName, IntraVMConnection clientConnection)
    throws Exception
  {
    if (traceSpace.enabled) traceSpace.trace("sys$net", toString() + "/connectListener: swiftletName=" + swiftletName + ", clientConnection=" + clientConnection);
    ...
    {
      connection.close();
      if (traceSpace.enabled) traceSpace.trace("sys$net", toString() + "/ConnectionVetoException: " + cve.getMessage());
