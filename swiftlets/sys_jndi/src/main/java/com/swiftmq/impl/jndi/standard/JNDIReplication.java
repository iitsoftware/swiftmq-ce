...
public class JNDIReplication implements TimerListener
{
  ...
  private void fillEnv()
  {
    ...
    propList.setEntityAddListener(new EntityChangeAdapter(null)
    {
        ...
        throws EntityAddException
      {
        ...
        if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", JNDIReplication.this.toString() + "/onEntityAdd (environment-property): property=" + name);
      }
    });
    propList.setEntityRemoveListener(new EntityChangeAdapter(null)
    {
        ...
        throws EntityRemoveException
      {
        ...
        if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", JNDIReplication.this.toString() + "/onEntityRemove (environment-property): property=" + name);
      }
    });
  }
  ...
  public synchronized void keepalive()
  {
    ...
    {
      if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/keepalive, NameNotFoundException, ok!");
    } catch (Exception e1)
    {
  ...
  {
    if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/bind, name=" + name + ", value=" + value + "...");
    if (!connected)
      return;
    try
    {
    ...
    try
    {
      ...
      if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/bind, name=" + name + ", actual object=" + value + "...");
      ctx.bind(addPrefix(name), value);
    } catch (Exception e)
    {
      if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/bind, name=" + name + ", value=" + value + ", exception=" + e);
      sctx.logSwiftlet.logWarning("sys$jndi", toString() + "/bind, name=" + name + ", value=" + value + ", exception=" + e);
    }
    if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/bind, name=" + name + ", value=" + value + "...done");
  }
  ...
  public synchronized void unbind(String name)
  {
    ...
    {
      if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/unbind, name=" + name + ", exception=" + e);
      sctx.logSwiftlet.logWarning("sys$jndi", toString() + "/unbind, name=" + name + ", exception=" + e);
    }
    if (sctx.traceSpace.enabled) sctx.traceSpace.trace("sys$jndi", toString() + "/unbind, name=" + name + "...done");
  }
