...
public class MessageQueue extends AbstractQueue
{
      ...
      throws QueueException
  {
    ...
    try
    {
      ...
      if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "startQueue: NonPersistentStore=" + nStore);
  ...
  public Object buildPreparedTransaction(PrepareLogRecord record) throws QueueException
  {
    ...
    try
    {
      if (!running)
        throw new QueueException("queue " + getQueueName() + " is not running");
      if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "buildPreparedTransaction, record: " + record);
      ...
      while (me == null)
      {
        ...
        try
        {
          ...
          while (iterator.hasNext())
          {
            ...
            {
              if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "Not expired, not selected: " + storeId);
            }
            storeId = null;
            me = null;
          }
        ...
        } catch (Exception e2)
        {
          ...
          if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "Exception cache.get: " + e2.getMessage());
        ...
        while (me == null)
        {
          ...
          try
          {
            ...
            while (iterator.hasNext())
            {
              ...
              {
                if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "Expired, not selected: " + storeId);
              }
              storeId = null;
              me = null;
            }
  ...
  public void setConsumerMode(int consumerMode)
  {
    ...
    try
    {
      if (ctx.queueSpace.enabled) ctx.queueSpace.trace(getQueueName(), "setConsumerMode, consumerMode=" + consumerMode);
