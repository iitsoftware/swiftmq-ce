...
import com.swiftmq.impl.routing.single.smqpr.v400.*;
...
public class XADeliveryStage extends Stage {
    ...
    protected void init() {
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.v400.SMQRFactory.ROUTE_REQ, request -> {
            if (ctx.traceSpace.enabled)
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.v400.SMQRFactory.ADJUST_REQ, request -> {
            if (ctx.traceSpace.enabled)
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.SMQRFactory.DELIVERY_REQ, request -> {
            ...
            try {
                ...
                XidImpl xid = new XidImpl(recoveryBranchQB, txNo.get(), (txBase + txNo.get()).getBytes());
                xid.setRouting(true);
                destTx.prepare(xid);
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.v400.SMQRFactory.COMMIT_REQ, request -> {
            if (ctx.traceSpace.enabled)
                ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), XADeliveryStage.this.toString("OUTBOUND") + "/visited, request=" + request + "...");
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.v400.SMQRFactory.TRANSACTION_REQ, request -> {
            if (ctx.traceSpace.enabled)
                ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), XADeliveryStage.this.toString("INBOUND") + "/visited, request=" + request + "...");
        ...
        visitor.setRequestHandler(com.swiftmq.impl.routing.single.smqpr.v400.SMQRFactory.COMMIT_REPREQ, request -> {
            if (ctx.traceSpace.enabled)
                ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), XADeliveryStage.this.toString("INBOUND") + "/visited, request=" + request + "...");
    ...
    public void close() {
        ...
        if (!outboundTransactions.isEmpty()) {
            ...
            outboundTransactions.forEach((xid, transaction) -> {
                try {
                    XAContext xac = ctx.xaResourceManagerSwiftlet.createXAContext(xid);
                    int id = xac.register(toString());
                    xac.addTransaction(id, transaction.getQueueName(), transaction);
        ...
        if (!inboundTransactions.isEmpty()) {
            ...
            inboundTransactions.forEach((key, tx) -> tx.handOver());
            if (ctx.traceSpace.enabled)
    ...
    public String toString(String direction) {
        return routingConnection.toString() + "/v400XADeliveryStage " + direction + ", recoveryBranchQ=" + recoveryBranchQ;
    }

    public String toString() {
        return routingConnection.toString() + "/v400XADeliveryStage, recoveryBranchQ=" + recoveryBranchQ;
    }
    ...
    private class Tx {
        ...
        void prepare() throws Exception {
            for (Iterator<Map.Entry<String, QueuePushTransaction>> iter = transactions.entrySet().iterator(); iter.hasNext(); ) {
                QueuePushTransaction t = iter.next().getValue();
                try {
                    t.prepare(xid);
                } catch (Exception e) {
                    iter.remove();
        ...
        void handOver() {
            try {
                XAContext xac = ctx.xaResourceManagerSwiftlet.createXAContext(xid);
                int id = xac.register(toString());
                for (Map.Entry<String, QueuePushTransaction> queuePushTransactionEntry : transactions.entrySet()) {
                    QueuePushTransaction t = queuePushTransactionEntry.getValue();
