...
public class DefaultScheduler extends Scheduler
{
  ...
  protected synchronized RoutingConnection getNextConnection()
  {
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == 0, rc=" + rc);
      return null;
    }
    if (connections.size() == 1)
    {
      ...
      if (rc.isClosed())
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == 1, rc is closed");
        connections.clear();
        rc = null;
      }
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == 1, rc=" + rc);
    } else
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == " + connections.size() + " ...");
      for (Iterator iter = connections.iterator(); iter.hasNext();)
      {
        rc = ((ConnectionEntry) iter.next()).getRoutingConnection();
        if (rc.isClosed())
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == " + connections.size() + ", rc is closed");
          iter.remove();
          rc = null;
        } else
          break;
      }
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == " + connections.size() + ", rc=" + rc);
    }
    if (rc == null && connections.size() == 0)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/getNextConnection, connections.size() == 0, rc=" + rc + ", destinationDeactivated");
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/addRoute, route=" + route);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/removeRoute, route=" + route);
    ConnectionEntry ce = getConnectionEntry(route.getRoutingConnection());
    if (ce != null)
    {
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/_removeRoutingConnection, routingConnection=" + routingConnection);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString() + "/removeRoutingConnection, routingConnection=" + routingConnection);
