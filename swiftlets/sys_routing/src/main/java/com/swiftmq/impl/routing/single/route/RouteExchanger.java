...
import com.swiftmq.impl.routing.single.manager.event.*;
...
import com.swiftmq.impl.routing.single.route.po.*;
...
public class RouteExchanger implements ConnectionListener, POExchangeVisitor,ActivationListener
{
  ...
  private void createFilterEntry(RouteFilter routeFilter, EntityList filterEntry)
  {
    ...
    filterEntry.setEntityAddListener(new EntityChangeAdapter(routeFilter)
    {
        ...
        throws EntityAddException
      {
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), RouteExchanger.this.toString()+"/onEntityAdd (routers): filter=" + myFilter + ", new entry=" + newEntity.getName());
      }
    });
    filterEntry.setEntityRemoveListener(new EntityChangeAdapter(routeFilter)
    {
        ...
        throws EntityRemoveException
      {
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), RouteExchanger.this.toString()+"/onEntityRemove (routers): filter=" + myFilter + ", del entry=" + delEntity.getName());
      }
    });
  }
  ...
  private void createFilter(Entity filterEntity)
  {
    ...
    if (filterType != null)
    {
      ...
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/creating filter for router: " + filterEntity.getName() + ", filter=" + routeFilter);
  ...
  private void createFilters(EntityList filterList)
  {
    ...
    filterList.setEntityAddListener(new EntityChangeAdapter(null)
    {
        ...
        throws EntityAddException
      {
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), RouteExchanger.this.toString()+"/onEntityAdd (filter): filter=" + newEntity.getName());
      }
    });
    filterList.setEntityRemoveListener(new EntityChangeAdapter(null)
    {
        ...
        throws EntityRemoveException
      {
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), RouteExchanger.this.toString()+"/onEntityRemove (filter): filter=" + delEntity.getName());
      }
    });
  }
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/activated, routingConnection="+routingConnection+" ...");
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/activated, routingConnection="+routingConnection+", exception enqueueRequest: "+e);
    }
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/activated, routingConnection="+routingConnection+" done");
  }

  public void processRoute(RoutingConnection routingConnection, Route route)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/processRoute, routingConnection="+routingConnection+", route="+route);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/connectionRemoved, evt="+evt);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/sendRoute, rc="+rc+", route= "+route+" ...");
    if (hopLimit > 0 && route.getHopCount() >= hopLimit)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/sendRoute, rc="+rc+", route= "+route+", route.getHopCount() > hopLimit");
      return;
    }
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/sendRoute, rc="+rc+", route= "+route+" sending...");
      try
      {
        rc.enqueueRequest(new SendRouteRequest(routeConverter.convert(route,rc.getProtocolVersion())));
      } catch (Exception e)
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/sendRoute, rc="+rc+", route= "+route+", exception enqueueRequest: "+e);
      }
    }
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/sendRoute, rc="+rc+", route= "+route+" done");
  }
  ...
  public void visit(PORouteObject po)
  {
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/visit, po="+po+", exception scheduling: "+e);
    }
  ...
  public void visit(POConnectionRemoveObject po)
  {
    ...
    if (routes != null)
    {
      for (int i=0;i<routes.size();i++)
      {
        ...
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), toString()+"/visit, po="+po+", exception while removing route from scheduler: "+e);
        }
        List connections = routeTable.getRoutingConnections();
        if (connections != null)
        {
