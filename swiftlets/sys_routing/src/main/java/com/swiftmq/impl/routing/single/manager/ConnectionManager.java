...
import com.swiftmq.impl.routing.single.manager.po.*;
  ...
  implements POCMVisitor
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionAdd...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionAdd done");
  }

  private void removeConnection(RoutingConnection rc, boolean mapRemove)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/removeConnection...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/removeConnection done");
  }

  private void removeAll()
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/removeAll...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/removeAll done");
  }

  public void visit(PORemoveObject por)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionRemove...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionRemove done");
  }

  public void visit(PORemoveAllObject po)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionRemoveAll...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/visitConnectionRemoveAll done");
  }
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/fireConnectionAdded...");
    synchronized (listeners)
    {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/fireConnectionAdded done");
  }

  private void fireConnectionRemoved(ConnectionEvent evt)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/fireConnectionRemoved...");
    synchronized (listeners)
    {
      for (int i = 0; i < listeners.size(); i++)
      {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.routingSwiftlet.getName(), "ConnectionManager/fireConnectionRemoved done");
  }
