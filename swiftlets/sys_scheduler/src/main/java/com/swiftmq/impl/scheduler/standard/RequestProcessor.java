...
public class RequestProcessor extends MessageProcessor
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/initSchedules ...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/initSchedules done");
  }
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/processException, exception="+e);
  }
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/sendReply, msg="+msg+" ...");
    ...
    t.commit();
    sender.close();
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/sendReply, msg="+msg+" done");
  }

  private void storeSchedule(MessageImpl msg) throws Exception
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/storeSchedule, msg="+msg+" ...");
    QueueSender sender = ctx.queueManager.createQueueSender(ctx.INTERNAL_QUEUE,null);
    QueuePushTransaction t = sender.createTransaction();
    t.putMessage(msg);
    t.commit();
    sender.close();
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/storeSchedule, msg="+msg+" done");
  }

  private void removeSchedule(String id) throws Exception
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/removeSchedule, id="+id+" ...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/removeSchedule, id="+id+" done");
  }

  public void run()
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/run, messageEntry="+messageEntry+" ...");
    try
    {
      ...
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/run, id="+id+", error="+error+", errorText="+errorText);
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/run, messageEntry="+messageEntry+" done");
  }
