  ...
  implements EventVisitor
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+" ...");
    try
    {
      if (hasJobFactory(scheduleEntry))
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+", has job factory...");
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+", jobStart="+scheduleEntry.jobStart);
        ...
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+", no job start!");
          String state = null;
      ...
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+", no job factory!");
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+", exception: "+e);
      try
      {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedule: "+scheduleEntry+" done");
  }

  private void calcSchedules(String groupName, String jobName)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedules, group: "+groupName+", job: "+jobName+" ...");
    for (Iterator iter=schedules.entrySet().iterator();iter.hasNext();)
    {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/calcSchedules, group: "+groupName+", job: "+jobName+" done");
  }

  private void stopSchedule(Entry scheduleEntry)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedule: "+scheduleEntry+" ...");
    if (scheduleEntry.job != null)
    {
      JobException jobException = null;
      try
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedule: "+scheduleEntry+" stopping job...");
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedule: "+scheduleEntry+" stopping job done");
      } catch (JobException e)
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedule: "+scheduleEntry+" stopping job, exception: "+e);
        if (scheduleEntry.schedule.isLoggingEnabled())
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedule: "+scheduleEntry+" done");
  }

  private void stopSchedules(String groupName, String jobName)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedules, group: "+groupName+", job: "+jobName+" ...");
    for (Iterator iter=schedules.entrySet().iterator();iter.hasNext();)
    {
      Entry scheduleEntry = (Entry)((Map.Entry)iter.next()).getValue();
      if (scheduleEntry.schedule.getJobGroup().equals(groupName) && scheduleEntry.schedule.getJobName().equals(jobName))
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedules, group: "+groupName+", job: "+jobName+" done");
  }

  private void stopSchedules()
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedules ...");
    for (Iterator iter=schedules.entrySet().iterator();iter.hasNext();)
    {
      Entry scheduleEntry = (Entry)((Map.Entry)iter.next()).getValue();
      stopSchedule(scheduleEntry);
    }
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/stopSchedules done");
  }
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" ...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" done");
  }
  ...
  public void visit(JobStart po)
  {
    ...
    if (entry != null)
    {
      if (entry.jobStart == po)
      {
        ...
        if (jobFactory != null)
        {
          try
          {
            ...
            if (po.getMaxRuntime() > 0)
            {
              ...
              if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" registering jobStop: "+entry.jobStop);
              try
              {
            ...
            if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" dispatching job: "+entry.job);
            runnerPool.dispatchTask(new JobRunner(po.getName(),entry.job,prop,entry,entry));
          } catch (Exception e)
          {
            if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+", exception: "+e);
            try
            {
        ...
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" no job Factory!");
        }
      } else
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" invalid in the meantime!");
      }
    } else
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" removed in the meantime!");
    }
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" done");
  }
  ...
  public void visit(JobStop po)
  {
    ...
    if (entry != null)
    {
      if (entry.jobStop == po)
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" stopping job: "+entry.job);
        entry.jobStop = null;
        ...
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" stopping job, exception: "+e);
      ...
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" rescheduled in the meantime!");
      }
    } else
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" removed in the meantime!");
  ...
  public void visit(JobTerminated po)
  {
    ...
    if (entry != null)
    {
      ...
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/"+po+" no further schedule allowed!");
        try
        {
          entry.entity.getProperty("state").setValue(STATE_JOB_EXCEPTION);
          entry.entity.getProperty("next-start").setValue(null);
  ...
  private class Entry implements JobTerminationListener
  {
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/jobTerminated, message="+s);
      pipelineQueue.enqueue(new JobTerminated(schedule.getName(),s));
    }

    public void jobTerminated(JobException e)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), toString() + "/jobTerminated, exception: "+e);
  ...
  private class JobRunner implements AsyncTask
  {
    ...
    public void run()
    {
      ...
      try
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), getDescription() + "/starting job ...");
        if (scheduleEntry.schedule.isLoggingEnabled())
        ...
        job.start(prop,jtl);
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), getDescription() + "/starting job done");
      } catch (JobException e)
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.schedulerSwiftlet.getName(), getDescription() + "/starting job, exception: "+e);
        if (scheduleEntry.schedule.isLoggingEnabled())
