...
import com.swiftmq.tools.deploy.*;
...
public class DeploySpaceImpl implements DeploySpace, TimerListener
{
  ...
  public Bundle[] getInstalledBundles() throws Exception
  {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/getInstalledBundles: " + list);
    if (list == null)
      return null;
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/setDeployListener: " + listener);
    if (listener == null)
    {
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/startTimer, instant timer listener, 1000");
    instant = true;
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction ...");
    if (closed)
      return;
    try
    {
      ...
      if (events != null)
      {
        ...
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, event=" + events[i]);
          ...
          {
            if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, " + bundle.getBundleName() + ", exception: " + e1);
      ...
      } else if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, no events!");

    } catch (Exception e)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, exception=" + e);
      ctx.logSwiftlet.logInformation(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, exception=" + e);
    }
    if (instant)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction, startTimer, interval=" + checkInterval);
      instant = false;
      ctx.timerSwiftlet.addTimerListener(checkInterval, this);
    }
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.deploySwiftlet.getName(), toString() + "/performTimeAction done");
  }
