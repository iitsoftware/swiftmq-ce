...
public class XALiveContextImpl extends XAContextImpl
{
  ...
  synchronized void registerUsageList()
  {
    if (registeredUsageList || closed || !prepared)
      return;
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.xaSwiftlet.getName(), toString() + "/registerUsageList...");
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.xaSwiftlet.getName(), toString() + "/registerUsageList done");
  }
