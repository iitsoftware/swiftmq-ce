    ...
    implements TimerListener
{
  ...
  public ThreadPool getPool(String threadName)
  {
    ...
    ThreadPool p = (ThreadPool) pools.get(name);
    if (traceSpace.enabled) traceSpace.trace(getName(), "getPoolForThreadName '" + threadName + "' returns " + name);
    return p;
  }
  ...
  protected void startup(Configuration config) throws SwiftletException
  {
    ...
    poolList.setEntityAddListener(new EntityChangeAdapter(null)
    {
          ...
          throws EntityAddException
      {
        ...
        if (traceSpace.enabled) traceSpace.trace(getName(), "onEntityAdd (pool), poolName=" + newEntity.getName());
      }
    });
    poolList.setEntityRemoveListener(new EntityChangeAdapter(null)
    {
          ...
          throws EntityRemoveException
      {
        ...
        if (traceSpace.enabled) traceSpace.trace(getName(), "onEntityRemove (pool): poolName=" + delEntity.getName());
      }
    });
    ...
    if (collectOn)
    {
      ...
      if (traceSpace.enabled) traceSpace.trace(getName(), "startup: collect interval <= 0; no thread count collector");
    }
  }
