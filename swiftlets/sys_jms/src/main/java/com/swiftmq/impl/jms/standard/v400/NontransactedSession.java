...
public class NontransactedSession extends Session
{
  ...
  public void visitAcknowledgeMessageRequest(AcknowledgeMessageRequest req)
  {
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/visitAcknowledgeMessageRequest, exception: " + e);
      if (reply != null)
      {
        reply.setOk(false);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/visitGenericRequest/RecoverSessionReply");
