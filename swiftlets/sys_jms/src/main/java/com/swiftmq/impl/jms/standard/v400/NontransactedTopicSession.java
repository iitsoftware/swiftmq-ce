...
public class NontransactedTopicSession extends NontransactedSession
{
  ...
  public void visitCloseConsumerRequest(CloseConsumerRequest req)
  {
    ...
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/visitCloseConsumerRequest, consumer closed due to client runtime exception: " + req.getException());
      ctx.logSwiftlet.logError("sys$jms", ctx.tracePrefix + "/visitCloseConsumerRequest, consumer closed due to client runtime exception: " + req.getException());
    }
  }
