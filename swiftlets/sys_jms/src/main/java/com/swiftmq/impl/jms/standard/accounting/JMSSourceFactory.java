...
public class JMSSourceFactory implements AccountingSourceFactory
{
  SwiftletContext ctx = null;
  Map parms = null;

  public JMSSourceFactory(SwiftletContext ctx)
  {
    this.ctx = ctx;
    this.ctx = ctx;
    parms = new HashMap();
    ...
    parms.put(p.getName(), p);
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.jmsSwiftlet.getName(), "JMSSourceFactory/created");
  }
  ...
  public AccountingSource create(Map map) throws Exception
  {
    ...
    return new JMSSource(ctx, flushInterval,
