...
public class TransactionManager {
    ...
    synchronized void commit() throws Exception {
        if (ctx.traceSpace.enabled)
            ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, transactions.size=" + transactions.size());
        for (Iterator iter = transactions.iterator(); iter.hasNext(); ) {
            QueueTransaction t = (QueueTransaction) iter.next();
            if (ctx.traceSpace.enabled)
                ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, t=" + t + ", closed=" + t.isClosed());
            try {
                t.commit();
                ...
                if (ctx.traceSpace.enabled)
                    ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, t=" + t + ", QueueTransactionClosedException, remove");
                iter.remove();
            }
        }
        startTransactions();
