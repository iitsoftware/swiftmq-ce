...
public class TransactionManager
{
    ...
    for (Iterator iter = transactionFactories.iterator(); iter.hasNext();)
    {
      ...
      if (!f.isMarkedForClose())
      {
        try
        {
          ...
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/startTransactions, add="+t+", closed="+t.isClosed());
          transactions.add(t);
        } catch (Exception e)
        {
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/startTransactions, e="+e+", remove transaction factory.");
          iter.remove();
        }
      }
      else
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, transactions.size="+transactions.size());
    for (Iterator iter = transactions.iterator(); iter.hasNext();)
    {
      QueueTransaction t = (QueueTransaction) iter.next();
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, t="+t+", closed="+t.isClosed());
      try
      {
        t.commit();
      } catch (QueueTransactionClosedException e)
      {
        ...
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$jms", ctx.tracePrefix + "/" + toString() + "/commit, t="+t+", QueueTransactionClosedException, remove");
        iter.remove();
      }
    }
    startTransactions();
