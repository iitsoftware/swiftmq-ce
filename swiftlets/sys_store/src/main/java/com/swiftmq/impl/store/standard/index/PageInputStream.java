...
public class PageInputStream implements LengthCaptureDataInput
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/flipPage, pageNo=" + pageNo + " ...");
    try
    {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/flipPage, pageNo=" + pageNo + " done");
  }
