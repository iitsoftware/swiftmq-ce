...
public class QueueIndex extends Index
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/createIndexPage, pageNo=" + pageNo);
  ...
  public QueueIndexEntry add(StoreEntry storeEntry, boolean referencable) throws Exception
  {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/add done, storeEntry=" + storeEntry);
    return entry;
  }
