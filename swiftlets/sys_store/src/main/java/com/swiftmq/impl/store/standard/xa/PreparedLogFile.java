...
public class PreparedLogFile extends PreparedLog
{
  ...
  public PreparedLogFile(StoreContext ctx, String path, boolean autoSync) throws IOException
  {
    ...
    getAll();
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/created, "+validRecords+" prepared transactions");
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/copy to "+(destPath+File.separatorChar+FILENAME)+" ...");
    ...
    destFile.getFD().sync();
    destFile.close();
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/copy to "+(destPath+File.separatorChar+FILENAME)+" done");
  }
