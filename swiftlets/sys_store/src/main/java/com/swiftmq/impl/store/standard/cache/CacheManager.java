...
public class CacheManager
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/freeSlots (before), size=" + nSlots);
    ...
    freePages.clear();
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/freeSlots (after), size=" + nSlots);
  }
  ...
  public Page fetchAndPin(int pageNo) throws Exception
  {
    ...
    try
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace("sys$store", toString() + "/fetchAndPin, pageNo=" + pageNo);
      getCount++;
      Slot slot = null;
      if (forceEnsure)
        ensure(pageNo);
