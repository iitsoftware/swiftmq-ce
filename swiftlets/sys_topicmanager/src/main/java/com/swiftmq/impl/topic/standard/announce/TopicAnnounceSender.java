...
import com.swiftmq.swiftlet.routing.event.*;
  ...
  implements POAnnounceSenderVisitor, RoutingListener, EntityWatchListener
{
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationAdded: " + event.getDestination());
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationActivated: " + event.getDestination());
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationDeactivated: " + event.getDestination());
    pipelineQueue.enqueue(new PODestinationDeactivated(event.getDestination()));
  }

  public void destinationRemoved(RoutingEvent event)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationRemoved: " + event.getDestination());
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/entityAdded: " + newEntity);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/entityRemoved: " + delEntity);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/versionNoteReceived: " + vn);
    pipelineQueue.enqueue(new POVersionNoteReceived(vn));
  }

  public void destinationAdded(Route route)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationAdded: " + route.getDestination());
    pipelineQueue.enqueue(new PODestinationAdded(route));
    if (route.isActive())
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/destinationAdded, activate: " + route.getDestination());
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/topicCreated: " + topicName);
    pipelineQueue.enqueue(new POTopicCreated(topicName));
  }

  public void topicRemoved(String topicName)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/topicRemoved: " + topicName);
    pipelineQueue.enqueue(new POTopicRemoved(topicName));
  }

  public void announceSubscriptions(TopicInfo remoteTI)
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/announceSubscriptions: " + remoteTI);
  ...
  {
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/send, dest: " + dest + ", vo: " + vo);
    dos.rewind();
  ...
  public void visit(POVersionNoteToSend po)
  {
    ...
    if (rtm != null)
    {
      ...
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/" + po + ", Exception: " + e);
      }
    }
  }
  ...
  public void visit(POVersionNoteReceived po)
  {
    ...
    if (!rtm.isValid())
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/" + po + ", RTM invalid after processBuffer, removing!");
      rtm.close();
    ...
    } else
    {
      if (rtm.getSender() == null)
      {
        try
        {
          ...
          if (!rtm.isValid())
          {
            if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/" + po + ", RTM object invalid, close!");
            rtm.close();
  ...
  public void visit(PODestinationActivated po)
  {
    ...
    } else if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/" + po + ", no rtm object found!");
  }
  ...
  public void visit(POTopicCreated po)
  {
    ...
    if (as == null)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(ctx.topicManager.getName(), toString() + "/" + po + ", announce all (createInfo)");
