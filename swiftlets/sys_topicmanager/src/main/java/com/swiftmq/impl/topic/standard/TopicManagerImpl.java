    ...
    implements QueueFactory
{
    ...
    topicList.setEntityAddListener(new EntityChangeAdapter(null)
    {
          ...
          throws EntityAddException
      {
        try
        {
          ...
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), "onEntityAdd (topics): new topic=" + name);
        } catch (Exception e)
        {
    ...
    topicList.setEntityRemoveListener(new EntityChangeAdapter(null)
    {
          ...
          throws EntityRemoveException
      {
        try
        {
          ...
          deleteTopic(name);
          if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), "onEntityRemove (topics): del topic=" + name);
        } catch (Exception e)
        {
  ...
  public synchronized void unsubscribe(int subscriberId)
  {
    if (subscriberId < topicSubscriptions.size())
    {
      ...
      {
        if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), "unsubscribe: topicSubscription = " + subscription);
  ...
  public synchronized void removeRemoteSubscriptions(String routerName)
  {
    ...
    if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), "removeRemoteSubscriptions: " + routerName + ", done");
  }
  ...
  public synchronized void processTopicInfo(TopicInfo topicInfo) throws Exception
  {
    ...
    if (broker == null)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), ": creating topic: " + topicInfo.getTopicName());
  ...
  synchronized void addSlowSubscriberCondition(SlowSubscriberCondition condition) throws Exception
  {
    ...
    if (broker == null)
    {
      if (ctx.traceSpace.enabled) ctx.traceSpace.trace(getName(), ": creating topic: " + condition.getTopicName());
