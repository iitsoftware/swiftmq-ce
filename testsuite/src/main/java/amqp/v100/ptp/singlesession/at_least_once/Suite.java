...
package amqp.v100.ptp.singlesession.at_least_once;
...
public class Suite extends ActiveTestSuite
{
  public static Test suite()
  {
    ...
    try
    {
      ...
      {
        suite.addTest(new Receiver("receive", QoS.AT_LEAST_ONCE, Util.getQueueNamePrefix() + (i + 1), connection, session, countDownLatch));
        suite.addTest(new Sender("send", QoS.AT_LEAST_ONCE, Util.getQueueNamePrefix() + (i + 1), connection, session));
      }
    } catch (Exception e)
    {
      e.printStackTrace();
