...
package jms.cc.nontransacted.clientack.psdur;
...
public class Tester extends PSTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            TopicSession qs = tc.createTopicSession(false, Session.CLIENT_ACKNOWLEDGE);
            qs.setMessageListener(new Listener(i));
            pool.addServerSession(new ServerSessionImpl(pool, qs));
        }
        cc = tc.createDurableConnectionConsumer(topic, "clientack", null, pool, 5);
