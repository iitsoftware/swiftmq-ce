...
package jms.ha.persistent.ptp.nontransacted.clientack.multiconsumer.multisession.nonrecover.onmessage;
...
public class Listener extends SimpleConnectedPTPTestCase {
    ...
    public void receive() {
        try {
            ...
            receiver.setMessageListener(new MyListener("testqueue", new MsgNoVerifier(this, nMsgs, "no"), sem1));
            Semaphore sem2 = new Semaphore();
            receiver1.setMessageListener(new MyListener("testqueue1", new MsgNoVerifier(this, nMsgs, "no"), sem2));
            Semaphore sem3 = new Semaphore();
            receiver2.setMessageListener(new MyListener("testqueue2", new MsgNoVerifier(this, nMsgs, "no"), sem3));
            sem1.waitHere();
            sem2.waitHere();
            sem3.waitHere();
            if (exception != null)
