...
/**
 * PubSub example to publish a request and receive a reply
 *
 ...
 * - Number of requests to publish [1]
 *
 ...
 * router1, publishes a request to the topic 'testtopic' and receives 
 * the reply.
 *
 * @author IIT GmbH, Bremen/Germany
 */
public class PubSubRequestor
{
	public static void main(String[] args)
	{
		String smqpURL = "smqp://localhost:4001";
		String tcfName = "plainsocket@router1";
		String topicName = "testtopic";
		int nMsgs = 1;
		if (args.length > 0 && args.length < 4)
		{
			System.out.println();
			System.out.println("Usage: java PubSubRequestor <smqpURL> <tcfName> <topicName> <nRequests>\n");
		...
		try {
			...
			// Create connection, session & publisher
			TopicConnection connection = connectionFactory.createTopicConnection();
			TopicSession session = connection.createTopicSession(false,Session.AUTO_ACKNOWLEDGE);
			TopicPublisher publisher = session.createPublisher(topic);
			publisher.setDeliveryMode(DeliveryMode.NON_PERSISTENT);
			
			// We don't use the javax.jms.TopicRequestor because it is bad designed.
			// It creates a new temp. topic for each requests and closes the session on close().
			// So we create one temp. topic for all the replies.
			TemporaryTopic replyTopic = session.createTemporaryTopic();
			TopicSubscriber replySubscriber = session.createSubscriber(replyTopic);
			...
			for (int i=0;i<nMsgs;i++)
			{
				...
				TextMessage reply = (TextMessage)replySubscriber.receive();
				System.out.println("Receiving: "+reply.getText());
			}
			
			// Close resources
			replyTopic.delete();
			replySubscriber.close();
			publisher.close();
			session.close();
			connection.close();
